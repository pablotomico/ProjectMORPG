...
void NetworkControl::RegisterClient(int l_clientID) {
	...
	msg.m_gameObjectIDs.first = l_clientID;
	Send(msg);
}
...
void NetworkControl::UpdateClient(int l_clientID, float l_x, float l_y) {
	GameObjectID goID = m_clientGameObjectMap[l_clientID];
	printf("Updating Client %d GameObject %d\n", l_clientID, goID);
	Message msg(MessageType::M_GameObject, System::S_GameObjectManager, goID, sf::Vector2f(l_x, l_y), sf::Time());
	Send(msg);
}
...
void NetworkControl::Notify(Message l_message) {
	...
	case MessageType::M_Player:
		...
		if (!HasClient(l_message.m_gameObject.m_gameObjectID)) {
			...
			UpdateClient(l_message.m_gameObject.m_gameObjectID, l_message.m_gameObject.m_position.x, l_message.m_gameObject.m_position.y);
		}

		break;
	case MessageType::M_GameObjectCreated:
		AddClientGameObject(l_message.m_gameObjectIDs.first, l_message.m_gameObjectIDs.second);
		break;
	}
}
